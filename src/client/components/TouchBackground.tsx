...
void main() {
    ...
    vec3 gradient = mix(vec3(1.0, 0.0, 0.0), vec3(0.0, 1.0, 0.0), uv.y);
    col *= gradient * uTint;
    col *= uBrightness;
...
function FaultyTerminal({
  ...
  // Initialize WebGL context only once when component mounts
  // Re-run only if fundamental setup props change (dpr, mouseReact)
  useEffect(() => {
    const ctn = containerRef.current;
    if (!ctn) return;
    ...
    return () => {
      ...
      // Properly dispose of WebGL resources using the extension
    ...
    dpr, // Only re-run if dpr changes (requires re-init)
    mouseReact, // Only re-run if mouseReact changes (requires adding/removing event listener)
    handleMouseMove // Include the memoized callback
  ]);

  // Separate effect to update uniforms that can change frequently
  // This effect runs independently of the main WebGL init effect
  useEffect(() => {
    ...
    // Note: pause and timeScale are handled in the main update loop
  }, [
    scale,
    gridMul,
    digitSize,
...
const FaultyTerminalM = React.memo(FaultyTerminal);
export default FaultyTerminalM;